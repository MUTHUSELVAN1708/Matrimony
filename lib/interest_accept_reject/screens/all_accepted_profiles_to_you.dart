...
class AllAcceptedProfilesToYou extends ConsumerStatefulWidget {
  const AllAcceptedProfilesToYou({super.key});

  @override
  ConsumerState<AllAcceptedProfilesToYou> createState() =>
      _RequestProfilesScreenState();
}

class _RequestProfilesScreenState
    extends ConsumerState<AllAcceptedProfilesToYou> {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
          ...
          title: Text(
            'All Accepted Requests To You (${interestModelState.sentInterests.where((model) => model.status == 'Accepted').length})',
      ...
      body: EnhancedLoadingWrapper(
        ...
        child: interestModelState.sentInterests
                .where((model) => model.status == 'Accepted')
                .isEmpty
            ? const Center(
                child: Text(
            ...
            : Container(
                ...
                child: ListView.builder(
                  itemCount: interestModelState.sentInterests
                      .where((model) => model.status == 'Accepted')
                      .length,
                  shrinkWrap: true,
                  itemBuilder: (context, index) {
                    final filteredInterests = interestModelState.sentInterests
                        .where((model) => model.status == 'Accepted')
                        .toList();
